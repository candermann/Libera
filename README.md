...
| `ADMIN_INITIAL_PASSWORD` | Nein | Startpasswort für `admin`, wird nur beim ersten Initialisieren genutzt. Muss mindestens 10 Zeichen lang sein. |
| `EXTRA_USERS_PASSWORD` | Nein | Startpasswort für `lehrer`, `schulleiter` und `sekretariat`, wird nur beim ersten Initialisieren genutzt. Standard ist `Bibliomat2024!`. |
...
EXTRA_USERS_PASSWORD=BitteAendern
```
...
- Es gibt vier Benutzer: `admin`, `lehrer`, `schulleiter`, `sekretariat`.
...
| Benutzer | Passwort (Erststart) | Gespeichert in |
|---|---|---|
| `admin` | `ADMIN_INITIAL_PASSWORD` (min. 10 Zeichen) | Tabelle `einstellungen` |
| `lehrer` | `EXTRA_USERS_PASSWORD` (Standard: `Bibliomat2024!`) | Tabelle `benutzer` |
| `schulleiter` | `EXTRA_USERS_PASSWORD` | Tabelle `benutzer` |
| `sekretariat` | `EXTRA_USERS_PASSWORD` | Tabelle `benutzer` |

Beim ersten Start werden die Passwörter aus den Umgebungsvariablen übernommen. Das Admin-Passwort kann danach über die Einstellungen geändert werden.
...
docker run -d \
  ...
  -e EXTRA_USERS_PASSWORD="BitteAendern" \
  bibliomat
```
