...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///schulbuch.db")
...
def _ensure_inventory_table(conn: Connection):
    ...
    conn.execute(
        text(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS uq_buch_zustand_preis
            ON buch_zustand_bestand (buch_id, zustand, verkaufspreis_cents)
            """
        )
    )
    conn.execute(
...
def _seed_initial_admin_password(conn: Connection):
    existing = conn.execute(
        text(
            ...
            SELECT wert
            FROM einstellungen
            WHERE schluessel = 'admin_password_hash'
            """
        )
    ).scalar()
