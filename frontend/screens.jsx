...
function SchuelerListe({ accent, onOpenStudent }) {
  ...
  const sortedStudents = React.useMemo(() => {
    ...
    return [...students].sort((a, b) => {
      ...
      else if (sortBy === 'name') { va = `${a.vorname} ${a.nachname}`; vb = `${b.vorname} ${b.nachname}`; }
              ...
              {rowsData.map((row, index) => {
                ...
                return (
                  <div key={`${row.row_number}-${index}`} style={{
                    ...
                    background: row.valid ? (isChecked ? '#f0fdf4' : 'transparent') : '#fff7ed',
                    ...
                    />
                    <div style={{ fontSize: 11.5, color: ok ? '#047857' : '#b45309' }}>
