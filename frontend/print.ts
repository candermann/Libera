...
function buildDocumentHTML({ titel, rechnungsNr, datum, student, posten, total, typ }: PrintDocumentData): string {
...
function addProtectedPreviewChrome(w: Window): void {
  const d = w.document;
...
function writeToPrintWindow(w: Window, html: string, autoPrint = false, options: PrintWindowOptions = {}): void {
  ...
  if (autoPrint) {
    setTimeout(() => { try { w.focus(); w.print(); } catch (_error) {} }, 300);
  }
}

function openPrintWindow(html: string, autoPrint = false): void {
...
async function openProtectedDocument(path: string, autoPrint = false, options: OpenProtectedDocumentOptions = {}): Promise<void> {
